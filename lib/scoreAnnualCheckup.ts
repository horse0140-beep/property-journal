...
export function buildAnnualReportSummary(params: {
  ...
  return `HomeWise Annual Home Health Report
${params.address}
...
<h1>HomeWise Annual Home Health Report</h1>
...
<p style="color:#8A9AB8;font-size:11px;margin-top:32px">Generated by HomeWise · ${new Date().toLocaleDateString()}</p>
</body></html>`;
}
...
export async function shareAnnualReportPdf(uri: string, address: string): Promise<{ error?: string }> {
  try {
    ...
    const dest = `${FileSystem.cacheDirectory}HomeWise_Annual_${safe}.pdf`;
