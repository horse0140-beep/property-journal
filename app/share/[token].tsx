    ...
    <Screen>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        <View style={{ alignItems: "center", marginBottom: 20 }}>
          ...
          <Text style={{ fontSize: 22, fontWeight: "900", color: colors.textPrimary, marginTop: 12, textAlign: "center" }}>
            {share.property_label}
          </Text>
          <Text style={{ color: colors.textMuted, marginTop: 4 }}>{share.label}</Text>
          ...
          {score ? (
            <View style={{ marginTop: 16, flexDirection: "row", alignItems: "center", gap: 12 }}>
              <View style={{
                ...
                <Text style={{ fontSize: 22, fontWeight: "900", color: colors.primary }}>{score.overall ?? "—"}</Text>
              </View>
              <View>
                <Text style={{ fontWeight: "800", color: colors.textPrimary }}>Home Health Score</Text>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <View style={{ flex: 1, minWidth: "28%", backgroundColor: colors.bgSection, borderRadius: 10, padding: 12, alignItems: "center" }}>
      <Text style={{ fontSize: 20, fontWeight: "900", color: colors.primary }}>{value}</Text>
      <Text style={{ color: colors.textMuted, fontSize: 11, textAlign: "center", marginTop: 4 }}>{label}</Text>
    </View>
  );
}
